...
function Select<V extends React.ReactText>({
  ...
  const handleKeyDown = useCallback(
    (event: React.KeyboardEvent) => {
      ...
      if (key === 'Enter') {
        toggle();
      } else if (isOpen && !isNil(cursor.next) && (key === 'ArrowRight' || key === 'ArrowDown')) {
        event.stopPropagation();
        onChange(options[cursor.next].value);
      } else if (isOpen && !isNil(cursor.prev) && (key === 'ArrowLeft' || key === 'ArrowUp')) {
        event.stopPropagation();
  ...
  const createClickChangeHandler = useCallback((newValue: V) => () => onChange(newValue), [
    onChange,
  ]);
