...
export function obfuscate(
  ...
  const numOfPlucksOnFirstIteration = numOfPlucksPerIteration + (str.length % rounds);

  for (let i = 1; i <= rounds; i++) {
    const numOfPlucks = i === 1 ? numOfPlucksOnFirstIteration : numOfPlucksPerIteration;
    const positionsToObfuscate = sampleSize(availablePositions, numOfPlucks);
    availablePositions = availablePositions.filter((p) => !positionsToObfuscate.includes(p));
